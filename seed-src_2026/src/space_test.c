...
#define MAX_TESTS 47 /*!< Total number of space tests */
...
int main(int argc, char **argv)
{
  ...
  if (all || test == 26)
    test2_space_destroy();
  if (all || test == 27)
    test1_space_get_objects_ids();
  if (all || test == 28)
    test2_space_get_objects_ids();
  if (all || test == 29)
    test1_space_get_number_objects();
  if (all || test == 30)
    test2_space_get_number_objects();
  if (all || test == 31)
...
{
  Space *s = NULL;
  PRINT_TEST_RESULT(space_has_character(s, 1) == FALSE);
}
