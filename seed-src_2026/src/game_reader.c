...
Status game_reader_load(Game *game, const char *filename)
{
  ...
  while (fgets(line, WORD_SIZE, file))
  {
    ...
    if (strncmp("#s:", line, 3) == 0)
    {
      ...
      if (game_reader_set_space(numspaces, game, idSpace, name, gdesc, file, space) == ERROR)
      {
        status = ERROR;
        break;
      }
    ...
    else if (strncmp("#o:", line, 3) == 0)
    {
      ...
      if (game_reader_set_object(numobjects, game, idSpace, name, obj_desc, obj_health, obj_movable, obj_dependency, obj_open, idObject, file, object, space) == ERROR)
      {
        status = ERROR;
        break;
      }
    ...
    else if (strncmp("#p:", line, 3) == 0)
    {
      ...
      if (game_reader_set_player(numplayers, game, idPlayer, name, player_gdesc, idSpace, health, backpack_capacity, file, player) == ERROR)
      {
        status = ERROR;
        break;
      }
    ...
    else if (strncmp("#c:", line, 3) == 0)
    {
      ...
      if (game_reader_set_character(numchr, game, idCharacter, name, character_gdesc, idSpace, health, friendly, char_msg, boss, file, character, space) == ERROR)
      {
        status = ERROR;
        break;
      }
    ...
    else if (strncmp("#l:", line, 3) == 0)
    {
      ...
      if (game_reader_set_link(numlinks, game, idLink, name, idSpace, idFinal, direction, open, file, link) == ERROR)
      {
        status = ERROR;
        break;
      }
